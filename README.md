...
- **FIM autocomplete** — opt-in local ghost text via `llama-server` (Settings; auto-starts when enabled)
...
Local fill-in-the-middle suggestions via `llama-server`. Off by default. When enabled for a loopback endpoint, the IDE can **spawn and own** the server (Start / Stop / exit only affect that owned process).

FIM client logic was adapted from [`vendor/llama.vscode`](vendor/llama.vscode/) ([ggml-org/llama.vscode](https://github.com/ggml-org/llama.vscode), **MIT**). GGUF trees are gitignored (not shipped on clone).

### 1. Model files

Place a FIM-capable GGUF under the **workspace root** (preferred):

- `codegemma-2b-GGUF/codegemma-2b-Q4_K_M.gguf`
- `Qwen2.5-Coder-1.5B-GGUF/Qwen2.5-Coder-1.5B.Q4_K_M.gguf`

If the open workspace has no copy, the IDE also looks under common h1code checkouts (`~/Documents/github/h1code/…`, `~/Documents/h1code/…`, etc.) so other projects can reuse those GGUFs without duplicating multi-GB files.

Catalog ids / relative paths: [`ui/src/fimModels.ts`](ui/src/fimModels.ts) / [`crates/ide-core/src/fim_models.rs`](crates/ide-core/src/fim_models.rs).

### 2. `llama-server` binary

Resolved in order:

1. `llama_server_path` in Settings / `.h1code/settings.toml` (absolute path)
2. `llama-server` on `PATH`
3. Common installs: `~/llama-cuda/llama-b*/`, `~/llama-b*/`, a few fixed layouts under `$HOME`

Many llama.cpp builds are **not** on PATH — leave the path empty if discovery finds yours, or set it explicitly:

```toml
# .h1code/settings.toml
autocomplete_enabled = true
# optional when discovery fails:
...
Open **Settings** via the topbar button, **File → Settings**, or `Ctrl+,`. Turn on autocomplete (or click **Start**). On workspace open — and again on UI bootstrap when a workspace is already open (`tauri dev` soft reload) — the IDE runs ensure and spawns if needed:

```bash
llama-server \
  -m "<resolved-model>.gguf" \
  --port 8081 -ngl 99 --ctx-size 0 -ub 512 -b 512 --cache-reuse 256
```

Spawn failures show in Settings status (and stderr under `~/.cache/` when available).

**Ownership:** if something healthy is already listening on the endpoint port, the IDE **reuses** it and does not claim ownership. **Start** / **Stop** and IDE exit only stop an IDE-spawned Child. A manually started server is left alone.

**`tauri dev`:** a Rust rebuild exits the shell and stops an owned server. After rebuild finishes, open the folder again (or use **Start**); Vite-only HMR keeps the shell workspace and calls ensure on bootstrap.

Manual launch (equivalent), from a directory that has the model:

```bash
./llama-server \
  -m "/path/to/codegemma-2b-GGUF/codegemma-2b-Q4_K_M.gguf" \
  --port 8081 -ngl 99 --ctx-size 0 -ub 512 -b 512 --cache-reuse 256
```
